...
void parent_func(int write_fd1, int read_fd2, int argc, char *argv[])
{
  struct stat st;
  ...
  for (int i = 1; i < argc; i++)
  {
    ...
    if (answer != 'R')
    {
      fprintf(stderr, "Ошибка. Неопределённый сигнал");
      exit(EXIT_FAILURE);
    }
    printf("Сигнал готовности принят\n");
    stat(argv[i], &st);
    ...
    printf("Метаданные файла %s отправлены\n", curFile.file_name);
    int bytes_to_write;
    while ((bytes_to_write = read(fd, buffer, BUFFER_SIZE)) > 0)
    {

      write(write_fd1, buffer, bytes_to_write);
      printf("%s отправлено %d байт\n", curFile.file_name, bytes_to_write);
    }
  ...
  printf("Команда к завершению отправлена\n");
  close(write_fd1);
  close(read_fd2);
  wait(&rv);
...
int main(int argc, char *argv[])
{
  ...
  default:
    ...
    parent_func(write_fd1, read_fd2, argc, argv);
    if (type == MKFIFO)
    {
      // unlink(buf_in);
      // unlink(buf_out);
